 ...
 * bytes: each FrameNN packet carries one Doom.FrameChunk struct and the
 * Palette packet carries the Doom.Palette struct. Raw packets are used
 * instead of parameter subscriptions because a 3200-element array
 * parameter encoded as protobuf-JSON is roughly 10x the size of its
 * binary form - at 80 chunks x 35 Hz that difference matters.
 *
 * The FrameNN/Palette packet ids are resolved at runtime from the YAMCS
 * MDB: the XTCE generated by fprime-to-xtce restricts each packet
 * container on the FPrimePacketId parameter, and containers holding a
 * Doom.FrameChunk / Doom.Palette parameter are identified by the type of
 * their first entry. Nothing in this file depends on deployment naming.
 */

// Must match Doom/Doom.fpp: FRAME_WIDTH, FRAME_HEIGHT.
...
// FrameChunk struct layout: frame u32, row u16, rowCount u16, width u16, pixels.
const CHUNK_PIXELS_OFFSET = 10;
...
class DoomDisplay {
    constructor() {
        this.instance = null;
        this.frameChunkPacketIds = new Set();
        this.palettePacketId = null;
        this.commands = {};      // suffix -> qualified name
        this.heldKeys = {};
        this.pixels = new Uint8Array(FRAME_WIDTH * FRAME_HEIGHT);
    ...
    /**
     * Identify FrameChunk/Palette packet containers in the MDB. A container
     * qualifies when it has a packet-id restriction (FPrimePacketId
     * comparison) and a parameter entry named FrameOutNN or PaletteOut.
     */
    async resolveMdb() {
        ...
        for (const container of containers) {
            ...
            if (kind === "frame") {
                this.frameChunkPacketIds.add(packetId);
            } else if (kind === "palette") {
                this.palettePacketId = packetId;
            }
        }
        if (this.frameChunkPacketIds.size === 0) {
            throw new Error("no FrameChunk packet containers in MDB");
        }
    ...
    onMessage(event) {
        ...
        if (this.frameChunkPacketIds.has(packetId)) {
            this.absorbChunk(bytes);
    ...
    draw() {
        ...
        for (let i = 0, o = 0; i < FRAME_WIDTH * FRAME_HEIGHT; i++, o += 4) {
        ...
        if (this.statusLine) {
            this.statusLine.textContent = "frame " + this.frameCount
                + " | " + this.fps.toFixed(1) + " fps | chunks " + this.chunksReceived
                + " | " + this.statusText;
        }
    }
